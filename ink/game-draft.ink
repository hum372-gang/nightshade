...
* [Exit: door]
    Thought: I don't {~|think I }need to {~leave|go out} {~yet|right now}.
+ (inspect_bed) {inspect_bed < 5} Inspect: bed
    { stopping:
...
+ (inspect_rdesk) {inspect_rdesk < 4 or not got_notebook} Inspect: rdesk
    { stopping:
        ...
        - { not letter_roommate_is_sick: Thought: Their notebook is so nice, too... }
            { letter_roommate_is_sick: Thought: I found their notebook! It's very nice. -> got_notebook -> }
        - { letter_roommate_is_sick and not got_notebook: Thought: They said I could borrow their notebook. I'll handle it carefully. -> got_notebook ->}
            { not letter_roommate_is_sick: Thought: I shouldn't snoop. }
    }
+ (inspect_desk) {inspect_desk < 3} Inspect: desk
    { stopping:
...
Wait: 3
Sound: school_bell
Thought: The bell's going off; it's time for the assembly.
